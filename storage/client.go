...
var (
	...
	defaultLoginURL = func(id string) string {
		fmt.Println("defaultLoginURL", id)
...
func (cc *ClientConfig) ToClient() *Client {
	return &Client{
		id:                             cc.ID,
		secret:                         cc.Secret,
		...
		accessTokenType:                op.AccessTokenType(cc.AccessTokenType),
		devMode:                        cc.DevMode,
		idTokenUserinfoClaimsAssertion: cc.IDTokenUserinfoClaimsAssertion,
		clockSkew:                      cc.ClockSkew,
	}
}
...
// no race conditions.
func RegisterClients(registerClients ...*ClientConfig) {
	for _, client := range registerClients {
		fmt.Printf("Registering client: %s\n", client.ID)
		fmt.Printf("%v\n", *client)
		clients[client.ID] = client.ToClient()
	}
}
