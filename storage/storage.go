...
func (s *Storage) SetUserinfoFromScopes(ctx context.Context, userinfo *oidc.UserInfo, userID, clientID string, scopes []string) error {
	fmt.Println("SetUserinfoFromScopes", userinfo, userID, clientID, scopes)
...
func (s *Storage) SetUserinfoFromToken(ctx context.Context, userinfo *oidc.UserInfo, tokenID, subject, origin string) error {
	...
	//	}
	//}
	fmt.Println("SetUserinfoFromToken:", tokenID, subject, origin)
...
func (s *Storage) setUserinfo(ctx context.Context, userInfo *oidc.UserInfo, userID, clientID string, scopes []string) (err error) {
	...
	for _, scope := range scopes {
		...
		case oidc.ScopeProfile:
			...
			l, er := language.Parse(user.PreferredLanguage)
			if er != nil {
				l = language.English
			}
			userInfo.Locale = oidc.NewLocale(l)
		case oidc.ScopePhone:
			userInfo.PhoneNumber = user.Phone
			userInfo.PhoneNumberVerified = user.PhoneVerified
		case CustomScope:
			fmt.Println("custom scope requested")
